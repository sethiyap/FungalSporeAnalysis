...
#'   feature file.
#' @param feature_gr A TxDb object of gene feature file, generated by \code{GenomicFeatures::makeTxDbFromGFF()}
#' @param genelist A tibble with two columns \code{1. gene_id and 2.Expression
#'   value} without header, to subset genes from the feature file. \code{default:NULL}
#' @param bw_files A character vector containing either name of the bw file object or names of bw file objects to plot.
...
#' @import grid unit
#'
#' @return Parallel plots of profiles(log2) for given gene lists, ordered by given expression value (high to low).
#'
#' @examples
#' \dontrun{
#'
...
#' genelist_specific_profileplot(feature_gr=feature_gr,bw_files = bw_files, genelist=gene_list, output_name="An_Pol2_TBP_TFIIB", ymin=3)
#'
#' }
genelist_specific_profileplot <- function(feature_gr, genelist=NULL, bw_files,promoter=TRUE, max_key=6, min_key=0, output=TRUE,output_name="Sample", palette="cream_brown", ymax=6,ymin=1){


          if(is.null(genelist)==TRUE){

                    sub_feature_gr <- GenomicFeatures::genes(feature_gr)
                    cluster_rows <- TRUE
          }
          else{
                    feature_gr <- GenomicFeatures::genes(feature_gr)
